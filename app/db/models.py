...
from sqlalchemy import Boolean, Column, Date, DateTime, ForeignKey, Integer, String, Text, UniqueConstraint, JSON, Float, event
from sqlalchemy.orm import DeclarativeBase, Mapped, mapped_column, relationship

# Import per auto-calcolo text_hash
...
# ✅ Event listener per auto-calcolare text_hash se mancante
@event.listens_for(Node, 'before_insert')
@event.listens_for(Node, 'before_update')
def auto_compute_text_hash(mapper, connection, target):
    """
    Calcola automaticamente text_hash prima di INSERT o UPDATE.
    
    Priorità:
    1. Se text_hash è già presente, non fa nulla
    2. Altrimenti calcola hash da text_clean (preferito)
    3. Se text_clean è vuoto, usa text_raw
    4. Se entrambi vuoti, usa hash di stringa vuota
    """
    if not target.text_hash:
        source_text = target.text_clean or target.text_raw or ""
        target.text_hash = compute_text_hash(source_text)
